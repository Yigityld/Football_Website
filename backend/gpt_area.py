...
def search_team_url(team_name: str) -> Optional[str]:
    ...
    try:
        ...
        print("[ERROR] Transfermarkt arama isteği zaman aşımına uğradı.")
        return None
    except Exception as e:
        print(f"[ERROR] Transfermarkt arama isteği başarısız: {e}")
        return None
...
def get_team_last_5_matches_with_tactics(team_name: str) -> Tuple[List[Dict], int, int, int]:
    def fetch_matches_from_url(url: str) -> List[Dict]:
        ...
        try:
            ...
            print("[ERROR] Transfermarkt maç isteği zaman aşımına uğradı.")
            return []
        except Exception as e:
            print(f"[ERROR] Transfermarkt maç isteği başarısız: {e}")
            return []
        soup = BeautifulSoup(response.text, "html.parser")
        div_responsive = soup.find("div", class_="responsive-table")
        # Linter fix: div_responsive Tag kontrolü
...
def get_last_matches(team_a: str, team_b: str) -> List[Dict]:
    ...
    try:
        ...
        print("[ERROR] Transfermarkt head-to-head isteği zaman aşımına uğradı.")
        return []
    except Exception as e:
        print(f"[ERROR] Transfermarkt head-to-head isteği başarısız: {e}")
        return []
    soup = BeautifulSoup(response.text, "html.parser")
    # Linter fix: table ve tbody Tag kontrolü
    ...
    for row in rows:
        ...
        try:
            ...
            # Güvenli home ve guest extraction (linter safe)
            home = cols[10].get_text(strip=True)
            if isinstance(cols[10], Tag) and hasattr(cols[10], 'find'):
                home_tag = cols[10].find('a')
                if isinstance(home_tag, Tag) and home_tag.has_attr('title'):
                    home = home_tag['title']
            guest = cols[8].get_text(strip=True)
            if isinstance(cols[8], Tag) and hasattr(cols[8], 'find'):
                guest_tag = cols[8].find('a')
                if isinstance(guest_tag, Tag) and guest_tag.has_attr('title'):
                    guest = guest_tag['title']
